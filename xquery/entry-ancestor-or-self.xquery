...
$workspace := prov:getPluginWorkspace(),
    ...
    $ancestor-or-self := function($entry as element()) as element()*
    {
        ($entry, $entry-hierarchy($entry))
    }
return if($is-entry($current-node)) then $ancestor-or-self($current-node) else ()